...
namespace Gigadrillgames.AUP.SpeechTTS
{
    public class TextToSpeechPlugin : MonoBehaviour
    {
        ...
        private Action<string, string> ErrorTTSSynthesize;

        public event Action<string, string> OnErrorTTSSynthesize
        {
            add { ErrorTTSSynthesize += value; }
            remove { ErrorTTSSynthesize -= value; }
        }

        private Action<string[]> GetTTSSynthesizeFiles;

        public event Action<string[]> OnGetTTSSynthesizeFiles
        {
            add { GetTTSSynthesizeFiles += value; }
            remove { GetTTSSynthesizeFiles -= value; }
        }
        ...
        public void Init()
        {
            ...
            if (Application.platform == RuntimePlatform.Android)
            {
                ...
                ttsCallback.OnInit = OnTTSInit;
                ttsCallback.OnGetLocaleCountry = OnTTSGetLocaleCountry;
                ttsCallback.OnSetLocale = OnTTSSetLocale;
                ttsCallback.OnStartSpeech = OnTTSStartSpeech;
                ttsCallback.OnDoneSpeech = OnTTSEndSpeech;
                ttsCallback.OnErrorSpeech = OnTTSErrorSpeech;
                ttsCallback.OnErrorTTSSynthesize = OnTTSErrorSynthesize;
                ttsCallback.OnGetSynthesizeFiles = OnTTSGetSynthesizeFiles;

                jo.CallStatic("init",currentActivity,ttsCallback);
                hasInit = true;
        ...
        private void SetTextToSpeechCallbackListener(
            Action<int> OnInit, Action<string> OnGetLocaleCountry, Action<int> OnSetLocale,
            Action<string> OnStartSpeech, Action<string> OnDoneSpeech, Action<string> OnErrorSpeech,
            Action<string, string> OnErrorTTSSynthesize, Action<string[]> OnGetSynthesizeFiles
        )
        {
#if UNITY_ANDROID
            if (Application.platform == RuntimePlatform.Android)
            {
                ...
                ttsCallback.OnErrorTTSSynthesize = OnErrorTTSSynthesize;
                ttsCallback.OnGetSynthesizeFiles = OnGetSynthesizeFiles;
        ...
        public void SynthesizeToFile(string whatToSay, string utteranceId, string fileName, string outputpath)
        {
#if UNITY_ANDROID
            if (Application.platform == RuntimePlatform.Android)
            {
                if (hasInit)
                {
                    jo.CallStatic("synthesizeToFile", whatToSay, utteranceId, fileName, outputpath);
                }
            }
            else
            {
        ...
        public void ResetMediaplayer()
        {
            ...
            if (Application.platform == RuntimePlatform.Android)
            {
                if (hasInit)
                {
                    jo.CallStatic("resetMediaplayer");
                }
            }
            else
            {
        ...
        public void GetSynthesizeFiles(string outputPath)
        {
#if UNITY_ANDROID
            if (Application.platform == RuntimePlatform.Android)
            {
                if (hasInit)
                {
                    jo.CallStatic("getSynthesizeFiles", outputPath);
                }
            }
            else
            {
        ...
        public void LoadSynthesizeFile(string fileName)
        {
#if UNITY_ANDROID
            if (Application.platform == RuntimePlatform.Android)
            {
                if (hasInit)
                {
                    jo.CallStatic("loadSynthesizeFile", fileName);
                }
            }
            else
            {
        ...
        public void PlaySynthesizeFile()
        {
            ...
            if (Application.platform == RuntimePlatform.Android)
            {
                if (hasInit)
                {
                    jo.CallStatic("playSynthesizeFile");
                }
            }
            else
            {
        ...
        private void OnTTSErrorSynthesize(string errorMessage, string utteranceId)
        {
            Debug.Log("[TextToSpeechDemo] OnErrorTTSSynthesize errorMessage: " + errorMessage);
            Debug.Log("[TextToSpeechDemo] OnErrorSpeech utteranceId: " + utteranceId);
        ...
        private void OnTTSGetSynthesizeFiles(string[] fileNames)
        {
            Debug.Log("[TextToSpeechDemo] OnTTSGetSynthesizeFiles errorMessage: " + fileNames);
            if (null != GetTTSSynthesizeFiles)
            {
                GetTTSSynthesizeFiles(fileNames);
            }
        }
    }
}