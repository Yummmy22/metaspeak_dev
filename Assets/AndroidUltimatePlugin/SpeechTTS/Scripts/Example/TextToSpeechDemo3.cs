...
{
    public class TextToSpeechDemo3 : MonoBehaviour
    {
        private const string TAG = "[TextToSpeechDemo3]: ";
        ...
        private int synthesizeCounter = 0;
        ...
        {
            if (inputField != null && fileNameInputField != null)
            {
                ...
                if (!String.IsNullOrEmpty(fileNameInputField.text))
                {
                    ...
                    if (textToSpeechPlugin.isInitialized())
                    {
                        // increase synthesizeCounter every click
                        synthesizeCounter++;
                        string utteranceId = "tts_synthesize_" + synthesizeCounter;
                        Debug.Log(TAG + "SynthesizeToFile utteranceId: " + utteranceId);

                        // un mute volume
                        utilsPlugin.UnMuteBeep();

                        UpdateStatus("Trying SynthesizeToFile...");
                        Debug.Log(TAG + "SynthesizeToFile whatToSay: " + whatToSay);
                        string outputPath = Application.persistentDataPath;
                        Debug.Log(TAG + "SynthesizeToFile outputPath: " + outputPath);
                        textToSpeechPlugin.SynthesizeToFile(whatToSay, utteranceId, fileName, outputPath);
                    }
                }
                else
                {
                    UpdateStatus("enter file name please!");
                }
            }
        }
        ...
        public void OnInputTextChange()
        {
            ...
            textToSpeechPlugin.ResetMediaplayer();
        }
        ...
        private void OnErrorTTSSynthesize(string errorMessage, string utteranceId)
        {
            dispatcher.InvokeAction(
                () =>
                {
                    UpdateStatus("OnErrorTTSSynthesize ...");

                    CancelInvoke("WaitingMode");
                    Invoke("WaitingMode", waitingInterval);

                    Debug.Log(TAG + "OnErrorTTSSynthesize errorMessage: " + errorMessage);
                    Debug.Log(TAG + "OnErrorTTSSynthesize utteranceId: " + utteranceId);
                }
            );
        }
    }
